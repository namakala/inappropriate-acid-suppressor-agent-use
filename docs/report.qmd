...
plogit2p <- function(x, n) {
  p <- meta::backtransf(x, sm = "PLOGIT")
  return(p)
}
...
printSummary <- function(mod, detail = FALSE) {
  ...
  if (!is.null(mod$bylevs)) {
    cat("**Subgroup-specific results:**\n\n")
    for (i in seq_along(mod$bylevs)) {
      lbl  <- mod$bylevs[i]
      p_i  <- plogit2p(mod$TE.random.w[i],    n = NULL) * 100
      lb_i <- plogit2p(mod$lower.random.w[i], n = NULL) * 100
      ub_i <- plogit2p(mod$upper.random.w[i], n = NULL) * 100
      n_i  <- mod$k.w[i]
      t2_i <- mod$tau2.w[i]
      i2_i <- mod$I2.w[i] * 100
      catf(
        "- **%s**: %.1f%% [%.1f%%, %.1f%%]  (k = %d, τ² = %.3f, I² = %.1f%%)\n",
        lbl, p_i, lb_i, ub_i, n_i, t2_i, i2_i
      )
    }
  ...
  catf(
    "**%s: %.3f [%.3f, %.3f]**  \n",
    res$text.random,
    plogit2p(res$TE.random,    n = mod$n),
    plogit2p(res$lower.random, n = mod$n),
    plogit2p(res$upper.random, n = mod$n)
  )
...
printRefCat <- function(varname) {
  refs <- c(
    "Age"                = "Reference: <65",
    "Continent"          = "Reference: North America",
    "JBI_Classification" = "Reference: High Quality",
    "Setting"            = "Reference: Hospital Setting",
    "use_guideline"      = "Reference: Followed Guideline(s)",
    "Year_ordinal"       = "Reference: <2015"
  )
  if (varname %in% names(refs)) {
    catf("*%s*  \n\n", refs[varname])
  }
}
...
```{r sens-table, results = "asis"}
tar_load(sens_overall_tbl)

knitr::kable(
  sens_overall_tbl,
  booktabs = TRUE,
  col.names = c(
    "Model", "k", "N", "Prevalence (95% CI)",
    "$\\tau^2$", "$I^2$", "95% PI", "$\\Delta$ Prev (pp)"),
  align = "lrrrrrrr"
)
```
...
for (obj in obj_subgroups) {
  ...
  tar_load(obj_copas)
  cat("```\n\n")
  copas_list <- get(obj_copas)
  for (nm in names(copas_list)) {
    catf("**%s**\n\n", nm)
    summary(copas_list[[nm]]) |> print()
    cat("\n\n")
  }
  cat("```\n\n")

  sens_tbl <- gsub(x = obj, "mod_subgroup", "sens_subgroup_tbl")
  tar_load(sens_tbl)
  cat("\n**Sensitivity Analysis**\n\n")
  print(knitr::kable(
    get(sens_tbl),
    booktabs = TRUE,
    col.names = c("Model", "k", "N", "Prevalence (95% CI)",
                  "$\\tau^2$", "$I^2$", "95% PI", "$\\Delta$ Prev (pp)"),
    align = "lrrrrrrr"
  ))
  cat("\n\n")
...
for (obj in obj_metareg_uni) {
  catf("\n### Model: `%s`\n\n", obj)
  var_name <- gsub("mod_metareg_", "", obj)
  printRefCat(var_name)
  tar_load(obj)
  cat("```\n\n")
